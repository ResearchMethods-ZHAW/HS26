...
exp(glm_poisson$coefficients["(Intercept)"])

# Anzahl besucher bei 30°C
exp(glm_poisson$coefficients["(Intercept)"] + 30 * glm_poisson$coefficients["Temperatur"]) 
...
exp(glm_logistic$coefficients["temperatur"])

# LD50 (also hier: Temperatur, bei der 50% der Touristen baden)
-glm_logistic$coefficients["(Intercept)"] / glm_logistic$coefficients["temperatur"]
...
ld50_male <- 
  -(glm_binomial_2$coefficients["(Intercept)"] + glm_binomial_2$coefficients["sexmale"] ) /
  glm_binomial_2$coefficients["ldose"]
...
```{r}
# Visualisierung
# geom_smooth funktioniert hier nicht (Gewichte ntotal werden ignoriert) wir 
# predicten die Kurven deshalb manuell (siehe Statistik 3)

grid <- 
  expand.grid(ldose = seq( min(budworm$ldose), max(budworm$ldose), length = 100),
  sex = unique(budworm$sex))

grid$pred <- predict(glm_binomial_2, newdata = grid, type = "response")

ggplot() +
  geom_point(data = budworm, aes(ldose, ndead / ntotal, color = sex)) +
  geom_line(data = grid, aes(ldose, pred, color = sex)) +
  labs(x = "Insektizidosis [µg]", y = "Sterbewahrscheinlichkeit") +
  scale_x_continuous(breaks = log2(c(1, 2, 4, 8, 16, 32)),
