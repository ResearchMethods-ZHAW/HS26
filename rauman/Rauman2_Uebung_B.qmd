...
In [der letzten Übung](#sec-raster-intro1) habt ihr nun eine erste Erfahrung mit Rasterdaten gemacht. Dabei haben wir zuerst einen Vektordatensatz rasterisiert. Häufig arbeiten wir aber mit Geodaten, die bereits im Rasterformat erfasst werden. 
...
Das sieht schon ziemlich toll aus, aber vielleicht wollen wir die Standard-Farbpalette ändern. Ein nettes GUI, das einem die Auswahl der Farben erleichtern soll, erhält ihr mit dem Befehl `cols4all::c4a_gui()`.
...
```{r}
#| output: true
#| fig-cap: Für diese Visualisierung verwende ich `tmap` und als Farbpalette `values = "matplotlib.cividis"`.
