...
In dieser Demo möchten wir weitere Werkzeuge aus dem Tidyverse vorstellen und mit Beispielen illustrieren. Die tidyverse-Tools erleichtern den Umgang mit Daten erheblich und haben sich mittlerweile zu einem "must have" im Umgang mit Daten in R entwickelt.
...
Nun kann die **convenience Variable** "Month" erstellt werden. Ohne `dplyr` wird eine neue Spalte folgendermassen hinzugefügt. 
...
2) Bilde Gruppen pro Monat  (`group_by(wetter,month)`) 
...
Dieses Verketten mittels `|>` (genannt "pipe") macht den Code einiges schreib- und leserfreundlicher, und wir werden ihn in den nachfolgenden Übungen verwenden. Der `|>` Operator ist seit R 4.1 in Base-R enthalten. Alternativ gibt es den `%>%` Operator aus dem `magrittr` Package.

Zu `dplyr` gibt es etliche Tutorials online (siehe^[@wickham2017, Kapitel 10 / <http://r4ds.had.co.nz/transform.html>, oder [Hands-on dplyr tutorial..](https://youtu.be/jWjqLW-u3hc)]), deshalb werden wir diese Tools nicht in allen Details erläutern. Nur noch folgenden wichtigen Unterschied zu zwei wichtigen Funktionen in `dplyr`: `mutate()` und `summarise()`.
...
Im Befehl `pivot_longer()` müssen wir festlegen, welche Spalten zusammengefasst werden sollen (hier: `temp_max`,`temp_min`). Alternativ können wir angeben, welche Spalten wir *nicht* zusammenfassen wollen:
...
Die ersten 6 Zeilen von `weather_summary`:
...
Beachte: `weather_summary_long` umfasst 62 Beobachtungen (Zeilen), das sind doppelt so viel wie `weather_summary`, da wir ja zwei Spalten zusammengefasst haben.
